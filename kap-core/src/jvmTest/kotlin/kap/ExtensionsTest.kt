...
class ExtensionsTest {
    ...
    fun `void discards result and returns Unit`() = runTest {
        val result = Async { Kap { 42 }.discard() }
    ...
    fun `void propagates failure`() = runTest {
        val result = runCatching {
            Async { Kap<Int> { throw RuntimeException("boom") }.discard() }
        }
    ...
    fun `settled wraps success in Result success`() = runTest {
        val result = Async { Kap { 42 }.settled() }
    ...
    fun `settled wraps failure in Result failure`() = runTest {
        val result = Async {
            Kap<Int> { throw IllegalStateException("bad") }.settled()
        }
        assertTrue(result.isFailure)
    ...
    fun `settled does not catch CancellationException`() = runTest {
        ...
        val job = launch { Async.invoke<Result<Int>> { comp } }
        started.await()
        job.cancel()
        job.join()
    ...
    fun `zipLeft propagates failure from right side`() = runTest {
        val result = runCatching {
            Async {
                Kap { delay(100); "left" }
                    .keepFirst(Kap<String> { throw RuntimeException("right failed") })
            }
        }
        assertTrue(result.isFailure)
        ...
        val result = runCatching {
            Async {
                Kap<String> { throw RuntimeException("left failed") }
                    .keepSecond(Kap { delay(100); "right" })
            }
        }
        assertTrue(result.isFailure)
