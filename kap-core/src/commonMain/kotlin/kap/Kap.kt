...
/**
 * Unwraps a callable that produces a [Kap]. Enables the `kap(myFn::kap)` form
 * generated by `@KapTypeSafe` on functions:
 *
 * ```
 * @KapTypeSafe fun myFn(a: String, b: Int): Result = ...
 *
 * // KSP generates:  fun ((String, Int) -> Result).kap(): Kap<...>
 *
 * kap(::myFn)         // overload taking a function ref directly
 * kap(myFn::kap)      // overload taking a bound reference to the .kap() extension
 * myFn.kap()          // direct call (no wrapper)
 * ```
 *
 * The three forms are interchangeable. `kap(myFn::kap)` is useful when you
 * already think of `.kap()` as a method on the function (callable reference
 * style); `kap(::myFn)` mirrors `kap(::DataClass)` for symmetry with classes.
 */
inline fun <X> kap(builder: () -> Kap<X>): Kap<X> = builder()
