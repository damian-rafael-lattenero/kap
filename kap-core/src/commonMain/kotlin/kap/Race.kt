 ...
 * ```
 * race(
 *     fa = Kap { fetchFromPrimary() },
 *     fb = Kap { fetchFromFallback() }
 * )
 * ```
 */
@OptIn(ExperimentalCoroutinesApi::class)
fun <A> race(fa: Kap<A>, fb: Kap<A>): Kap<A> = Kap {
    supervisorScope {
 ...
 */
@OptIn(ExperimentalCoroutinesApi::class)
fun <A> raceN(vararg computations: Kap<A>): Kap<A> {
 ...
 */
fun <A> Iterable<Kap<A>>.raceAll(): Kap<A> =
 ...
 *     .raceAgainst(Kap { fetchFromReplica() })
 * ```
 */
fun <A> Kap<A>.raceAgainst(other: Kap<A>): Kap<A> =
    race(this, other)
