...
/**
 * Acquires a resource, uses it in a [Kap], and guarantees [release]
 ...
 * @param use function that builds a [Kap] from the acquired resource.
 ...
 * ```
 */
fun <A> Kap<A>.guarantee(finalizer: suspend () -> Unit): Kap<A> = Kap {
    try {
...
sealed class ExitCase {
    /** Kap completed successfully. */
    data class Completed<out A>(val value: A) : ExitCase()

    /** Kap failed with an exception. */
 ...
 * ```
 */
fun <A> Kap<A>.guaranteeCase(finalizer: suspend (ExitCase) -> Unit): Kap<A> = Kap {
    try {
 ...
 * bracketCase(
 *     acquire = { openConnection() },
 *     use = { conn -> Kap { conn.query("SELECT ...") } },
 *     release = { conn, case ->
 *         when (case) {
