...
fun <E, A> invalid(e: E): Kap<Either<NonEmptyList<E>, A>> = Kap.of(Either.Left(nonEmptyListOf(e)))

/** Wraps multiple errors into a validated computation. */
fun <E, A> invalidAll(errors: NonEmptyList<E>): Kap<Either<NonEmptyList<E>, A>> = Kap.of(Either.Left(errors))
...
): Kap<Either<NonEmptyList<E>, B>> = withV(Kap { fa() })
 ...
 */
infix fun <E, A, B> Kap<Either<NonEmptyList<E>, (A) -> B>>.thenV(
    fa: Kap<Either<NonEmptyList<E>, A>>,
): Kap<Either<NonEmptyList<E>, B>> {
    val self = this
...
infix fun <E, A, B> Kap<Either<NonEmptyList<E>, (A) -> B>>.thenV(
    fa: suspend () -> Either<NonEmptyList<E>, A>,
): Kap<Either<NonEmptyList<E>, B>> = thenV(Kap { fa() })
 ...
 */
infix fun <E, A, B> Kap<Either<NonEmptyList<E>, (A) -> B>>.thenValueV(
    fa: Kap<Either<NonEmptyList<E>, A>>,
...
infix fun <E, A, B> Kap<Either<NonEmptyList<E>, (A) -> B>>.thenValueV(
    fa: suspend () -> Either<NonEmptyList<E>, A>,
): Kap<Either<NonEmptyList<E>, B>> = thenValueV(Kap { fa() })

// ── catching: bridge from exception world to validated world ─────────────

/**
 * Converts a [Kap] into a validated computation by catching
 * non-cancellation exceptions and mapping them to errors via [toError].
 */
fun <E, A> Kap<A>.catching(toError: (Throwable) -> E): Kap<Either<NonEmptyList<E>, A>> =
    Kap {
        try {
 ...
 */
fun <E, A> Kap<A>.validate(toError: (A) -> E?): Kap<Either<NonEmptyList<E>, A>> =
    Kap {
 ...
 */
fun <E, A> Kap<A>.ensureV(
    error: (A) -> E,
    predicate: (A) -> Boolean,
): Kap<Either<NonEmptyList<E>, A>> = Kap {
...
fun <E, A, B> Iterable<A>.traverseV(
    f: (A) -> Kap<Either<NonEmptyList<E>, B>>,
): Kap<Either<NonEmptyList<E>, List<B>>> = Kap {
 ...
 */
fun <E, A> Iterable<Kap<Either<NonEmptyList<E>, A>>>.sequenceV(): Kap<Either<NonEmptyList<E>, List<A>>> =
    traverseV { it }

/**
 * Like [sequenceV] but limits the number of concurrent computations.
 */
fun <E, A> Iterable<Kap<Either<NonEmptyList<E>, A>>>.sequenceV(
    concurrency: Int,
): Kap<Either<NonEmptyList<E>, List<A>>> =
 ...
 */
fun <E, A> Kap<Either<NonEmptyList<E>, A>>.recoverV(
    f: (Throwable) -> E,
): Kap<Either<NonEmptyList<E>, A>> = Kap {
    try {
 ...
 */
fun <E, A> Kap<Either<NonEmptyList<E>, A>>.orThrow(): Kap<A> = map {
    when (it) {
 ...
 */
fun <E, A, B> Kap<Either<NonEmptyList<E>, A>>.mapV(f: (A) -> B): Kap<Either<NonEmptyList<E>, B>> =
    map { it.map(f) }
 ...
 */
fun <E, F, A> Kap<Either<NonEmptyList<E>, A>>.mapError(f: (E) -> F): Kap<Either<NonEmptyList<F>, A>> =
    map { either ->
        when (either) {
...
class ValidatedScope<E> @PublishedApi internal constructor(
    ...
    /**
     * Executes a validated [Kap] and unwraps the result — returns the
     * [Right][Either.Right] value or short-circuits with the [Left][Either.Left] errors.
     */
    suspend fun <A> Kap<Either<NonEmptyList<E>, A>>.bindV(): A =
 ...
 */
fun <E, A> validated(block: suspend ValidatedScope<E>.() -> A): Kap<Either<NonEmptyList<E>, A>> =
    Kap {
 ...
 */
fun <E, A> accumulate(block: suspend ValidatedScope<E>.() -> A): Kap<Either<NonEmptyList<E>, A>> =
    validated(block)
