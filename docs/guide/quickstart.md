...
!!! info "KSP enables `@KapTypeSafe`"
    The KSP plugin and `kap-ksp` processor generate named builder methods (`.withUser {}`, `.thenStock {}`, etc.) from your `@KapTypeSafe`-annotated data classes. If you only need the generic `.with {}` / `.then {}` API, the `kap-core` dependency alone is sufficient.
...
!!! note "Generic `.with {}` still works"
    The generic `.with { }` API is still valid and equivalent. Named builders are syntactic sugar generated by KSP that make chains more readable.
