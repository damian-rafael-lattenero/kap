...
!!! info "KSP enables `@KapTypeSafe`"
    The KSP plugin and `kap-ksp` processor generate a scoped wrapper with per-slot tag interfaces from your `@KapTypeSafe`-annotated data classes. Inside `.with { field from value }` the lambda's implicit receiver narrows the IDE autocomplete to the field expected at the current curry position — swapping fields is a compile error. The core `.with {}` / `.then {}` operators are reused internally. For advanced composition, you can also use `Kap.of { }` with manual currying.
...
suspend fun main() {
    ...
    val result: Dashboard = kap(::Dashboard)
        ...
        .with { cart from fetchCart() }      // │ total time = max(50, 40, 30) = 50ms
        .with { promos from fetchPromos() }  // ┘ not 120ms sequential
        .evalGraph()
    println(result)
...
That's it. `@KapTypeSafe` generates `.with { user from  }`, `.with { cart from  }`, `.with { promos from  }` from your data class properties. Three calls run in parallel, results are type-checked into your data class, and each step is self-documenting.
...
suspend fun main() {
    ...
    val result = kap(CreateUser)
        ...
        .with { email from "alice@example.com" }
        .with { age from 30 }
        .evalGraph()
    println(result)
    ...
    val dashboard: EnrichedDashboard = kap(::UserContext)
        ...
        .andThen { ctx ->                            // ── barrier: ctx available
            kap(::EnrichedDashboard)
                ...
                .with { promos from fetchPromotions(ctx.prefs) }        // ┘ uses ctx from phase 1
        }
        .evalGraph()
    println(dashboard)
...
// The wrapper carries the curried function shape; type inference handles it
// for you, so the explicit annotations below are just for documentation.
fun addCart(
    partial: OrderKap<(OrderCart) -> (OrderShipping) -> Order>,
    type: String,
): OrderKap<(OrderShipping) -> Order> = when (type) {
    "premium" -> partial.with { cart from fetchPremiumCart() }
    else      -> partial.with { cart from fetchStandardCart() }
}

// Another function adds shipping based on country
fun addShipping(
    partial: OrderKap<(OrderShipping) -> Order>,
    country: String,
): OrderKap<Order> = when (country) {
    "AR" -> partial.with { shipping from calcShippingAR() }
    "US" -> partial.with { shipping from calcShippingUS() }
    else -> partial.with { shipping from 0.0 }
}
